...
def build_lesson_and_quiz_prompts(skill, concept, module_desc="", previous_wrong=None):
    ...
    if skill == "remember" or skill == "remembering":
        ...
        quiz_prompt = (
            ...
            "State internally the answer at the end, but don't show it to the user., e.g. 'Answer: B'."
        )
        ...
        quiz_prompt = (
            ...
            f"Write an easy fill-in-the-blank multiple-choice (A-D) question specifically about {context}. "
            "Example: 'The main function of a job scheduler in HPC is to ___ (A: manage jobs, B: edit scripts, ...)'. State internally the answer at the end, but don't show it to the user."
        )
        ...
        quiz_prompt = (
            ...
            f"Give a very short command or script completion MCQ (A-D) based on the example above about {context}. "
            "If the answer is code, include hints. Example: 'Which line submits the job? A: sbatch myjob.sh ...' State internally the answer at the end, but don't show it to the user."
        )
        ...
        quiz_prompt = (
            context +
            f"Ask a multiple-choice (A-D) question about {context} where the user has to spot the error or choose the better option, using the material in the description. "
            "Example: 'Which of these two scripts will run efficiently? Why?' State internally the answer at the end, but don't show it to the user."
        )
        ...
        quiz_prompt = (
            context +
            f"Ask: 'Which would you choose and why?' as a multiple-choice (A or B) about {context}. "
        ...
        quiz_prompt = (
            f"Present a partially completed script or workflow from the module {context}. Ask the learner to fill in a missing line (MCQ, A-D) that achieves a specific task described in the module. "
            "State internally the answer at the end, but don't show it to the user."
        )
    else:
        lesson_prompt = (
        ...
        quiz_prompt = (
            f"Ask a single multiple-choice (A-D) question to recall a fact from the following concept {context}. State internally the answer at the end, but don't show it to the user."
        )
