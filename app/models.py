...
class Progress(db.Model):
    ...
    learning_level = db.Column(db.String(32))
    user_level = db.Column(db.String(32))  # <-- Add this new line
    ...
    quiz_type = db.Column(db.String(10), default='mcq')
...
class LearningObjective(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    module_key = db.Column(db.String(10), nullable=False)    # e.g. "A1"
    level = db.Column(db.String(32), nullable=False)         # e.g. "apply"
    description = db.Column(db.Text, nullable=False)
    prerequisites = db.Column(db.Text)  # Store as JSON string
    knowledge = db.Column(db.Text)
    course = db.Column(db.Text)
    external = db.Column(db.Text)
    __table_args__ = (
        db.UniqueConstraint('module_key', 'level', name='uq_module_level'),
    )
