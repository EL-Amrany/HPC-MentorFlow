...
def user_has_prereqs(user, prerequisites):
    """
    prerequisites: list of strings like "B5 understand: 60"
    Returns True if user meets *all* of them.
    """
    for req in prerequisites:
        # Split "B5 understand: 60" → ["B5 understand", " 60"]
        parts = req.split(':')
        if len(parts) != 2:
            return False
        mod_lev, min_score = parts[0].strip(), int(parts[1].strip())
        mod_key, level = mod_lev.split()
        mod = Module.query.filter_by(key=mod_key).first()
        if not mod:
            return False
        prog = Progress.query.filter_by(user_id=user.id, module_id=mod.id).first()
        # Here we assume quiz_passed==True means they “scored” sufficiently
        if not prog or not prog.quiz_passed:
            return False
    return True
